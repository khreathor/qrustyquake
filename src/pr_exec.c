...
{ // Returns the new program statement counter
	...
	int c = f->locals; // save off any locals that the new function steps on
	...
	localstack_used += c;
	int o = f->parm_start; // copy parameters
...
int PR_LeaveFunction()
{
	...
	int c = pr_xfunction->locals; // restore locals from the stack
	localstack_used -= c;
...
{ // The interpretation main loop
	...
	int runaway = 100000;
	pr_trace = false;
	int exitdepth = pr_depth; // make a stack frame
	...
	while (1) {
		...
		case OP_EQ_V:
			...
			break;
		case OP_EQ_S:
			c->_float = !strcmp(PR_GetString(a->string),
		...
		case OP_CALL8:
			...
			if (newf->first_statement < 0) { // negative statements
				int i = -newf->first_statement; // are builtins
				if (i >= pr_numbuiltins)
