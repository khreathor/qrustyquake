...
#define	ANIM_CYCLE	2
		...
		for (j = 0; j < max; j++) {
			tx2 = anims[j];
			if (!tx2)
				Sys_Error("Missing frame %i of %s", j,
					  tx->name);
			...
			aliashdr_t *pheader)
{
	...
	*pskinindex = (byte *) pskin - (byte *) pheader;
	...
	if (r_pixbytes == 1) {
		...
		Sys_Error
		    ("Mod_LoadAliasSkin: driver set invalid r_pixbytes: %d\n",
		     r_pixbytes);
	}
...
void *Mod_LoadSpriteFrame(void *pin, mspriteframe_t **ppframe)
{
	...
	pspriteframe =
	    Hunk_AllocName(sizeof(mspriteframe_t) + size * r_pixbytes,
			   loadname);
	...
	*ppframe = pspriteframe;
	...
	if (r_pixbytes == 1) {
		Q_memcpy(&pspriteframe->pixels[0], (byte *) (pinframe + 1),
			 size);
	} else if (r_pixbytes == 2) {
		...
		Sys_Error
		    ("Mod_LoadSpriteFrame: driver set invalid r_pixbytes: %d\n",
		     r_pixbytes);
	}
