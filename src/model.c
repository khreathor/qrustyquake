...
=================
*/
/*TODOstatic void Mod_LoadEntities (lump_t *l)
{
...
static void Mod_BoundsFromClipNode (model_t *mod, int hull, int nodenum)
{
	mplane_t	*plane;
	mclipnode_t	*node;
...
// load into heap
	...
	if (mod->bspversion == BSPVERSION && external_vis.value && sv.modelname[0] && !strcasecmp(loadname, sv.name))
	{
		FILE* fvis;
