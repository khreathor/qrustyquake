...
#define MAX_MOD_KNOWN 1024 // CyanBun96: was 256
#define ANIM_CYCLE 2
...
void *Mod_Extradata(model_t *mod) // Caches the data if needed
{
...
void Mod_ClearAll()
{
	...
	for (; i < mod_numknown; i++, mod++) {
		...
		if (mod->type == mod_sprite) //FIX FOR CACHE_ALLOC ERRORS:
			mod->cache.data = NULL;
	}
}
...
void Mod_LoadTextures(lump_t *l)
{
	...
	for (i = 0; i < m->nummiptex; i++) { // sequence the animations
		...
		for (j = 0; j < max; j++) { // link them all together
			tx2 = anims[j];
			if (!tx2)
...
void Mod_LoadVertexes(lump_t *l)
{
	...
	mvertex_t *out = Hunk_AllocName(count * sizeof(*out), loadname);
...
void Mod_LoadEdges(lump_t *l)
{
	...
	if (l->filelen % sizeof(*in))
		Sys_Error("MOD_LoadBmodel: funny lump size in %s",
				loadmodel->name);
	int count = l->filelen / sizeof(*in);
	medge_t *out = Hunk_AllocName((count + 1) * sizeof(*out), loadname);
	loadmodel->edges = out;
...
void Mod_LoadNodes(lump_t *l)
{
	...
	for (int i = 0; i < count; i++, in++, out++) {
		...
		for (int j = 0; j < 2; j++) {
			...
			out->children[j] = p >= 0 ? loadmodel->nodes + p
...
void Mod_LoadLeafs(lump_t *l)
{
	...
	if (l->filelen % sizeof(*in))
		Sys_Error("MOD_LoadBmodel: funny lump size in %s",
				loadmodel->name);
	int count = l->filelen / sizeof(*in);
	mleaf_t *out = Hunk_AllocName(count * sizeof(*out), loadname);
	loadmodel->leafs = out;
...
void Mod_MakeHull0()
{
	...
	for (int i = 0; i < count; i++, out++, in++) {
		...
		for (int j = 0; j < 2; j++) {
			...
			out->children[j] = child->contents < 0 ? child->contents
				: child - loadmodel->nodes;
		}
	}
}
...
void Mod_LoadSurfedges(lump_t *l)
{
	...
	if (l->filelen % sizeof(*in))
		Sys_Error("MOD_LoadBmodel: funny lump size in %s",
				loadmodel->name);
	int count = l->filelen / sizeof(*in);
	int *out = Hunk_AllocName(count * sizeof(*out), loadname);
...
void Mod_LoadPlanes(lump_t *l)
{
	...
	if (l->filelen % sizeof(*in))
		Sys_Error("MOD_LoadBmodel: funny lump size in %s",
				loadmodel->name);
	int count = l->filelen / sizeof(*in);
	mplane_t *out = Hunk_AllocName(count * 2 * sizeof(*out), loadname);
...
void Mod_LoadAliasModel(model_t *mod, void *buffer)
{
	...
	mdl_t *pmodel = (mdl_t *) ((byte *) & pheader[1] +
	...
	maliasskindesc_t *pskindesc = Hunk_AllocName(numskins*sizeof(maliasskindesc_t),loadname);
	...
	stvert_t *pstverts = (stvert_t *) & pmodel[1];
	...
	dtriangle_t *pintriangles = (dtriangle_t *) & pinstverts[pmodel->numverts];
	...
	if (numframes < 1)
		Sys_Error("Mod_LoadAliasModel: Invalid # of frames: %d\n",
				numframes);
	daliasframetype_t *pframetype = (daliasframetype_t *) &
		pintriangles[pmodel->numtris];
