...
void S_TransferStereo16(int endtime)
{
	...
	unsigned long *pbuf = (unsigned long *) shm->buffer;
...
void S_PaintChannels(int endtime)
{
	while (paintedtime < endtime) {
		int end = endtime; // if paintbuffer is smaller than DMA buffer
		if (endtime - paintedtime > PAINTBUFFER_SIZE)
			end = paintedtime + PAINTBUFFER_SIZE;
		// clear the paint buffer
		Q_memset(paintbuffer, 0,
			 (end - paintedtime) * sizeof(portable_samplepair_t));
		channel_t *ch = channels; // paint in the channels.
...
void SND_PaintChannelFrom8(channel_t *ch, sfxcache_t *sc, int count)
{
	...
	for (int i = 0; i < count; i++) {
		paintbuffer[i].left += lscale[sfx[i]];
		paintbuffer[i].right += rscale[sfx[i]];
	}
	ch->pos += count;
}
...
void SND_PaintChannelFrom16(channel_t *ch, sfxcache_t *sc, int count)
{
	...
	for (int i = 0; i < count; i++) {
		int left = (sfx[i] * ch->leftvol) >> 8;
		int right = (sfx[i] * ch->rightvol) >> 8;
