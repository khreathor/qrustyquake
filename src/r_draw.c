...
void R_RenderPoly(msurface_t *fa, int clipflags)
{
	...
	while (pclip) { // clip the polygon, done if not visible
		...
		for (i = 0; i < lnumverts; i++) {
			float dist = DotProduct(verts[vertpage][i].position,
					pclip->normal) - pclip->dist;
	...
	r_nearzi = 0;
	for (i = 0; i < lnumverts; i++) {
		vec3_t local, transformed; // transform and project
...
void R_ZDrawSubmodelPolys(model_t *pmodel)
{
	...
	for (int i = 0; i < numsurfaces; i++, psurf++) {
		...
		if (((psurf->flags & SURF_PLANEBACK) // draw the polygon
			&& (dot < -BACKFACE_EPSILON))
				|| (!(psurf->flags & SURF_PLANEBACK)
				&& (dot > BACKFACE_EPSILON)))
			R_RenderPoly(psurf, 15); // FIXME: use bounding-box-based frustum clipping info?
	}
}
