...
// the finale prints the characters one at a time
	if (cl.intermission)
		remaining =
		    scr_printspeed.value * (cl.time - scr_centertime_start);
	else
		remaining = 9999;
...
void SCR_DrawRam(void)
{
	...
	Draw_PicScaled(scr_vrect.x + 32 * uiscale, scr_vrect.y, scr_ram,
		       uiscale);
}
...
void SCR_DrawNet(void)
{
	...
	Draw_PicScaled(scr_vrect.x + 64 * uiscale, scr_vrect.y, scr_net,
		       uiscale);
}
...
// decide on the height of the console
	...
	if (scr_conlines < scr_con_current) {
		scr_con_current -=
		    scr_conspeed.value * host_frametime * uiscale;
		if (scr_conlines > scr_con_current)
			scr_con_current = scr_conlines;
