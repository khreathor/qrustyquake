...
qboolean SV_CheckBottom(edict_t *ent) // Returns false if any part of the bottom
{ // of the entity is off an edge that is not a staircase.
	vec3_t mins, maxs, start, stop;
	VectorAdd(ent->v.origin, ent->v.mins, mins);
	VectorAdd(ent->v.origin, ent->v.maxs, maxs);
	// if all of the points under the corners are solid world, don't
	// bother with the tougher checks
...
realcheck:
	c_no++; // check it for real...
	start[2] = mins[2]; // the midpoint must be within 16 of the bottom
	...
	for (int x = 0; x <= 1; x++) // corners must be within 16 of midpoint        
		for (int y = 0; y <= 1; y++) {
			start[0] = stop[0] = x ? maxs[0] : mins[0];
			start[1] = stop[1] = y ? maxs[1] : mins[1];
			trace = SV_Move(start, vec3_origin, vec3_origin,
					stop, true, ent);
...
// stairs, but if the move isn't possible, no move is done, false is returned,
// and pr_global_struct->trace_normal is set to the normal of the blocking wall
qboolean SV_movestep(edict_t *ent, vec3_t move, qboolean relink)
{
	vec3_t oldorg, neworg, end;
	VectorCopy(ent->v.origin, oldorg); // try the move 
	...
	if ((int)ent->v.flags & (FL_SWIM | FL_FLY)) {
		...
		for (int i = 0; i < 2; i++) {
			...
			trace_t trace = SV_Move(ent->v.origin, ent->v.mins,
					ent->v.maxs, neworg, false, ent);
			if (trace.fraction == 1) {
				if (((int)ent->v.flags & FL_SWIM)
					&& SV_PointContents(trace.endpos) ==
					CONTENTS_EMPTY)
	...
	if (!SV_CheckBottom(ent)) {
		...
		// mostly pulled out from underneath it and is trying to correct
			if (relink)
				SV_LinkEdict(ent, true);
			return true;
		}
...
qboolean SV_StepDirection(edict_t *ent, float yaw, float dist)
{
	...
	if (SV_movestep(ent, move, false)) {
		...
		if (delta > 45 && delta < 315) // not turned far enough so don't
			VectorCopy(oldorigin, ent->v.origin); // take the step
		SV_LinkEdict(ent, true);
		return true;
	}
	SV_LinkEdict(ent, true);
