...
cvar_t sv_aim = { "sv_aim", "0.93", false, false, 0, NULL };
...
char *PF_VarString(int first) // Builtins
{
	static char out[256];
	out[0] = 0;
...
void SetMinMaxSize(edict_t *e, float *min, float *max, qboolean rotate)
{
	...
	if (!rotate) {
		...
		float *angles = e->v.angles; // find min / max for rotations
...
void PF_traceline()
{
	...
	pr_global_struct->trace_ent = trace.ent ? EDICT_TO_PROG(trace.ent)
...
int PF_newcheckclient(int check)
{
	vec3_t org;
	int i;
	if (check < 1) // cycle to the next one
		check = 1;
...
void PF_precache_file() { G_INT(OFS_RETURN) = G_INT(OFS_PARM0); }
...
void PF_lightstyle()
{
	...
	if (sv.state != ss_active) // send message to all clients on this server
		return;
	int j = 0;
...
void PF_rint()
{
	...
	G_FLOAT(OFS_RETURN) = f > 0 ? (int)(f + 0.5) : (f - 0.5);
}
...
void PF_setspawnparms()
{
	...
	client_t *cl = svs.clients + (i - 1); // copy spawn parms from client_t
...
void PF_changelevel()
{
	if (svs.changelevel_issued) // make sure we don't issue two changelevels
		return;
...
builtin_t pr_builtin[] = {
	...
	PF_vectoangles, PF_WriteByte, PF_WriteChar, PF_WriteShort, PF_WriteLong,
	PF_WriteCoord, PF_WriteAngle, PF_WriteString, PF_WriteEntity,
	PF_Fixme, PF_Fixme, PF_Fixme, PF_Fixme, PF_Fixme, PF_Fixme, PF_Fixme,
	SV_MoveToGoal, PF_precache_file, PF_makestatic, PF_changelevel,
	PF_Fixme,
	PF_cvar_set, PF_centerprint, PF_ambientsound, PF_precache_model,
