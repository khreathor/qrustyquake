...
void SV_ConnectClient(int clientnum)
{
	...
	netconnection = client->netconnection; // set up the client_t
	if (sv.loadgame)
...
void SV_CheckForNewClients()
{
	...
	while (1) { // check for new connections
		...
		if (!ret)
			break;
		int i = 0; // init a new client structure
...
// send over all entities (excpet the client) that touch the pvs
        ...
        for (e=1 ; e<sv.num_edicts ; e++, ent = NEXT_EDICT(ent))
        {
                ...
                if (msg->cursize + 40 > msg->maxsize)
                {
                        ...
                        //FIXMEif (!dev_overflows.packetsize || dev_overflows.packetsize + CONSOLE_RESPAM_TIME < realtime )
                        //FIXME{
                              //FIXME  Con_Printf ("Packet overflow!\n");
                                //FIXMEdev_overflows.packetsize = realtime;
                        //FIXME}
                        goto stats;
                        //johnfitz
                }
...
// send an update
                ...
                //FIXMEif ((ent->baseline.effects ^ (int)ent->v.effects) & pr_effects_mask)
                        //FIXMEbits |= U_EFFECTS;
                ...
                //johnfitz -- alpha
              //FIXME  if (pr_alpha_supported)
                //FIXME{
                        // TODO: find a cleaner place to put this code
                      //FIXME  val = GetEdictFieldValue(ent, "alpha");
                       //FIXME if (val)
                             //FIXME   ent->alpha = ENTALPHA_ENCODE(val->_float);
                //FIXME}

                //don't send invisible entities unless they have effects
                //FIXMEif (ent->alpha == ENTALPHA_ZERO && !((int)ent->v.effects & pr_effects_mask))
                      //FIXME  continue;
                //johnfitz
        ...
        //
                ...
                //FIXMEif (bits & U_EFFECTS)
                        //FIXMEMSG_WriteByte (msg, (int)ent->v.effects & pr_effects_mask);
                if (bits & U_ORIGIN1)
        ...
        //johnfitz -- devstats
stats:
        //FIXMEif (msg->cursize > 1024 && dev_peakstats.packetsize <= 1024)
                //FIXMECon_DWarning ("%i byte packet exceeds standard limit of 1024 (max = %d).\n", msg->cursize, msg->maxsize);
        //FIXMEdev_stats.packetsize = msg->cursize;
        //FIXMEdev_peakstats.packetsize = q_max(msg->cursize, dev_peakstats.packetsize);
        //johnfitz
}
...
void SV_SendClientMessages()
{
	...
	int i; // build individual updates;
	for (i = 0, host_client = svs.clients; i < svs.maxclients; i++, host_client++){
		...
		if (host_client->spawned) {
			...
			// keepalive messages if too much time has passed send
			// a full message when the next signon stage has been
...
//
        ...
        sv.protocol = PROTOCOL_FITZQUAKE; // johnfitz
