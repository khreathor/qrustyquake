		...
		float attenuation)
{
	...
	if (sv.datagram.cursize > MAX_DATAGRAM - 16)
		return;
	int sound_num = 1; // find precache number for sound;
	...
	for (int i = 0; i < 3; i++)
		MSG_WriteCoord(&sv.datagram, entity->v.origin[i] + 0.5 *
				(entity->v.mins[i] + entity->v.maxs[i]));
}
...
void SV_SendServerinfo(client_t *client)
{
	...
	for (s = sv.sound_precache + 1; *s; s++)
		MSG_WriteString(&client->message, *s);
	MSG_WriteByte(&client->message, 0);
	MSG_WriteByte(&client->message, svc_cdtrack); // send music
	MSG_WriteByte(&client->message, sv.edicts->v.sounds);
	MSG_WriteByte(&client->message, sv.edicts->v.sounds);
	MSG_WriteByte(&client->message, svc_setview); // set view 
	...
	while (1) {
		...
		if (node->contents < 0) {
			if (node->contents != CONTENTS_SOLID) {
				byte *pvs = Mod_LeafPVS((mleaf_t *) node,
							sv.worldmodel);
...
{
	vec3_t org;
	VectorAdd(clent->v.origin, clent->v.view_ofs, org) // find client's PVS
...
void SV_WriteClientdataToMessage(edict_t *ent, sizebuf_t *msg)
{
	...
	if (val)
		...
		items = (int)ent->v.items | ((int)pr_global_struct->
					serverflags << 28);
	bits |= SU_ITEMS;
...
void SV_SpawnServer(char *server) // This is called at the start of each level
{
	...
	ent = EDICT_NUM(0); // load the rest of the entities
