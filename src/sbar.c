...
void Sbar_Init(void)
{
	...
	for (i = 0; i < 5; i++) {
		sb_weapons[2 + i][0] =
		    Draw_PicFromWad(va("inva%i_shotgun", i + 1));
		sb_weapons[2 + i][1] =
		    Draw_PicFromWad(va("inva%i_sshotgun", i + 1));
		sb_weapons[2 + i][2] =
		    Draw_PicFromWad(va("inva%i_nailgun", i + 1));
		sb_weapons[2 + i][3] =
		    Draw_PicFromWad(va("inva%i_snailgun", i + 1));
		sb_weapons[2 + i][4] =
		    Draw_PicFromWad(va("inva%i_rlaunch", i + 1));
		sb_weapons[2 + i][5] =
		    Draw_PicFromWad(va("inva%i_srlaunch", i + 1));
		sb_weapons[2 + i][6] =
		    Draw_PicFromWad(va("inva%i_lightng", i + 1));
	}
...
void Sbar_DrawPic(int x, int y, qpic_t *pic)
{
	if (cl.gametype == GAME_DEATHMATCH)
		Draw_PicScaled(x * uiscale /* + ((vid.width - 320)>>1) */ ,
			       y * uiscale + (vid.height -
					      SBAR_HEIGHT * uiscale), pic,
			       uiscale);
	else
...
{
	if (cl.gametype == GAME_DEATHMATCH)
		Draw_TransPicScaled(x * uiscale /*+ ((vid.width - 320)>>1) */ ,
				    y * uiscale + (vid.height -
						   SBAR_HEIGHT * uiscale), pic,
				    uiscale);
	else
		Draw_TransPicScaled(x * uiscale +
...
{
	if (cl.gametype == GAME_DEATHMATCH)
		Draw_CharacterScaled(x *
				     uiscale /*+ ((vid.width - 320)>>1) */  + 4,
				     y * uiscale + (vid.height -
						    SBAR_HEIGHT * uiscale), num,
				     uiscale);
	else
		Draw_CharacterScaled(x * uiscale +
				     ((vid.width - 320 * uiscale) >> 1) + 4,
				     y * uiscale + (vid.height -
						    SBAR_HEIGHT * uiscale), num,
				     uiscale);
}
...
{
	if (cl.gametype == GAME_DEATHMATCH)
		Draw_StringScaled(x * uiscale /*+ ((vid.width - 320)>>1) */ ,
				  y * uiscale + (vid.height -
						 SBAR_HEIGHT * uiscale), str,
				  uiscale);
	else
		Draw_StringScaled(x * uiscale +
				  ((vid.width - 320 * uiscale) >> 1),
				  y * uiscale + (vid.height -
						 SBAR_HEIGHT * uiscale), str,
				  uiscale);
...
// draw the text
	...
	for (i = 0; i < l; i++) {
		...
		Draw_Fill(xofs + x * 8 * uiscale + 10 * uiscale, y,
			  28 * uiscale, 4 * uiscale, top);
		...
		if (k == cl.viewentity - 1) {
			Sbar_DrawCharacter(x * 8 + 2 + (uiscale - 1) * 2, -24,
					   16);
			Sbar_DrawCharacter((x + 4) * 8 - 4 + (uiscale - 1) * 2,
					   -24, 17);
		}
		x += 4;
	}
...
void Sbar_IntermissionOverlay(void)
{
	...
	Draw_PicScaled((vid.width - 192 * uiscale) / 2, 24 * uiscale, pic,
		       uiscale);
...
// time
	...
	Draw_TransPicScaled(vid.width - pad - 86 * uiscale, 64 * uiscale,
			    sb_colon, uiscale);
	Draw_TransPicScaled(vid.width - pad - 74 * uiscale, 64 * uiscale,
			    sb_nums[0][num / 10], uiscale);
	Draw_TransPicScaled(vid.width - pad - 54 * uiscale, 64 * uiscale,
			    sb_nums[0][num % 10], uiscale);

	Sbar_IntermissionNumber(vid.width / 2, 104 * uiscale,
				cl.stats[STAT_SECRETS], 3, 0);
	Draw_TransPicScaled(vid.width - pad - 88 * uiscale, 104 * uiscale,
			    sb_slash, uiscale);
	Sbar_IntermissionNumber(vid.width - pad - 80 * uiscale, 104 * uiscale,
				cl.stats[STAT_TOTALSECRETS], 3, 0);

	Sbar_IntermissionNumber(vid.width / 2, 144 * uiscale,
				cl.stats[STAT_MONSTERS], 3, 0);
	Draw_TransPicScaled(vid.width - pad - 88 * uiscale, 144 * uiscale,
			    sb_slash, uiscale);
	Sbar_IntermissionNumber(vid.width - pad - 80 * uiscale, 144 * uiscale,
				cl.stats[STAT_TOTALMONSTERS], 3, 0);
